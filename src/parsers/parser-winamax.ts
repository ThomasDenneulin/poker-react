...
export function parse(rl: readline.ReadLine): Promise<Hand[]> {
    ...
    */
    ...
    const shouldGoToNextState: any = {
        0: (line: any) => _.includes(line, 'HandId'),
        1: _.constant(true),
        2: _.constant(true),
        3: (line: any) => _.includes(line, '***'),
        4: _.constant(true),
        5: (line: any) => _.includes(line, 'Dealt to'),
        6: _.constant(true),
        7: _.constant(true),
        8: (line: any) => _.includes(line, '***')
    };

    const shouldSkipLine: any = {
        0: _.constant(false),
        1: _.constant(false),
        2: _.constant(false),
        3: _.constant(false),
...
function parseRoundLine(hand: Hand, line: string): Hand {
    ...
    let cards: any = [];
    if (cardsString) {
        cards = _.map(cardsString.split(' '), (s: any) => new Card(s));
    }
...
function parseActionLine(hand: Hand, line: string): Hand {
    ...
    let player: any = new Player("aa", -1);
    player = _.find(hand.players, (p: any) => line.indexOf(p.name) > -1);
